...
from docs_freshness_check import __version__
from docs_freshness_check.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from docs_freshness_check.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='docs-freshness-check',
        description=(
                        'Audit docs inventories for stale owners, old updates, and broken review '
                        'cadence.'
                    ),
    )
    parser.add_argument("--version", action="version", version=f"%(prog)s {__version__}")
    parser.add_argument("input", nargs="?", type=Path, help='documentation inventory')
    parser.add_argument(
        "--format",
